...
class GenerateStudentPaymentOrder:
    def __init__(self, nombre_estudiante, ci, correo, convocatoria_nombre, areas):
    ...
    def __calcular_total(self):
        return sum((area.get('monto') or 0.0) for area in self.areas)
    ...
    def generar_orden_pago(self):
        ...
        y -= 20
        c.setFont("Helvetica-Bold", 10)
        c.drawString(x_margin + 300, y, f"Total a pagar: Bs. {self.total:.2f}")

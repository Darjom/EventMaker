...
class DelegationInscriptionValidator:

    def __init__(
        self,
        ocr_service: OrquestadorOCRService,
        get_inscriptions_service: GetStudentInscriptionsByDelegation,
    ...
    def validate(self, image_invoice, delegation_id: int) -> str:
        invoice_number, invoice_total, invoice_url = self._extract_invoice_data(image_invoice)

        _, _, inscriptions = self.get_inscriptions_service.execute(delegation_id)
